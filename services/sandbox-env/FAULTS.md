...
| The Modal Sandbox itself (terminated, `NotFoundError: … container … not found`, `Task has already finished`, exec failed, upload failed) | `ESANDBOX` (was `EINTERNAL` before 2026-09-12 18:30) | `WorkspaceError` in `workspace.py` → `mcp_tools._fail` | `real/sandbox`, `outcome_known: true` (the call did not run) |
