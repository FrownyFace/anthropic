...
- `web-replay-story` a plain-English "what happened" bar (`role=region`, aria-label `What happened`) narrates the current checkpoint with prev/next; checkpoints are built only from structured event fields (`src/lib/story.ts`), the fault checkpoint is toned `fault` and names the file, a real failure is toned `real failure`. *(The five-panel verdict strip that preceded this was removed by another session on 2026-09-12 ~19:26 EDT; `src/lib/reliability.ts` still exists but is not rendered — see PLAN.md §3.3.2.)*
- `web-workspace` the workspace column is resizable (drag handle, collapsible to zero, split remembered), with Files / Diffs / Timeline / Logs tabs; Logs rows are virtualized and follow the tail while live.
- `web-identity` a fresh browser mints `u_<uuid4>` (localStorage + cookie mirror), every harness request carries `X-Faultline-User`, the rail shows only that identity's conversations, another identity gets 404 for the same conversation; the user menu can copy or reset the id.
...
- `web-failure-modes` an unknown run id shows "Could not load this run" (no provisioning spinner); an unreachable harness keeps replays usable and says so in the composer hint; a run that could not be graded shows why in the taxonomy's words.
...
- **Live conversation.** `--live` enables `F3` (*Run* on lost-ack from the table → `/conversations/:id?run=…`, rail entry, `live · sse`, terminal status badge, `sqlite` chip once persisted; `outputs/live.json` records `run_id` / `conversation_id` / `user_id` / `final_status`), `F4` (hard reload restores the transcript from `GET /conversations/{id}`: task bubble, steps, composer hint; `outputs/conversation.json` saved; another identity gets 404) and `F4b` (the persisted transcript shows the grader's verdict — **marked `test.fail()` while the persisted projection drops the evaluation**; the runner reports it `PASS` with the note "KNOWN GAP still present" until the fix lands, then Playwright flags it as passing unexpectedly and the marker must be removed).
...
- **Known defect (2026-09-12 19:55 EDT, owner anthropic-4c):** the Theme radiogroup is rendered inside Base UI's `DropdownMenuLabel`, which is `aria-hidden`, so `F9` fails on its accessibility assertion (controls invisible to screen readers although mouse users see them). Fix: render the row as a plain element inside the `DropdownMenuGroup`, not inside the label. Until then `flows.F9` is an honest FAIL, not a flake.
