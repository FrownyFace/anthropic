...
| `ThinkingTrace.tsx` | `ThinkingState` ("Steps") | Header shimmers while `active`, settles when `done`. Rows are `ToolCallView`s (tool icon, label, command/path, and a rail glyph from `lib/callStatus.ts` via the shared `STATUS_GLYPH` table: spinner, check, cross, question mark for an *unknown* outcome, ban sign for *not executed*). The badge trio (`CallBadges`: `FaultFiredBadge`, `ErrorOriginBadge`, `RecoveredBadge` — the last is the "read-back seen" hint) renders on the rows only when no chips are nested (`rowBadges`), so it appears once. Optional `thinking` paragraph; `children` render inside the expanded body. Stage timers removed; `defaultOpen` (else open while active). |
| `ToolCallChips.tsx` | `ToolChips` | One row per call with the icon→chevron hover swap and the mono chip. Expansion is inline (grid-rows reveal) instead of a hover portal so it works in a scrolling column. Body shows input, every result part (error + code, stdout, stderr, content, entries, bytes written, truncation), `exit n` / ok / error / no ack / not executed pill (`lib/callStatus.ts`, icons from `STATUS_GLYPH`), duration, the `CallBadges` trio, and the unified diff (`CodeBlock`) for a mutating call with a matching `FileDiff`. `+n −m` count on the row. |
